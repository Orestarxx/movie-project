...
export const tvService = {
    ...
    tvSeries: {
        ...
        getOnAirToday: async (page: string): Promise<IData & { results: ITelevision[] }> => {
            const onTheAir: IData & {
                results: ITelevision[]
            } = await fetch(baseURL + endPoints.onTheAir + query.page + page, {
                method: 'GET',
                headers: headers
            }).then(response => response.json())
        ...
        getPopularTV: async (page: string): Promise<IData & { results: ITelevision[] }> => {
            const popularTV: IData & {
                results: ITelevision[]
            } = await fetch(baseURL + endPoints.popularTV + query.page + page, {
                method: 'GET',
                headers: headers
            }).then(response => response.json())
